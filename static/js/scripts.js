...
function fillQuestionData (questions, index) {
	...
	for(let i = 0; i < optionLabels.length; i++){
		if (i == 0) {
			optionLabels[i].innerHTML = questions[index].correct;
		}else{
			optionLabels[i].innerHTML = questions[index].incorrect[i-1];	
		}				
	}
	...
	nextButton.onclick = function (){
			if(answerIsTrue(questions[index].correct)){
				userScore+=1;
			}
...
async function getQuestionDataFromServer(category, level){
	let url = `http://localhost:3000/api/questions?amount=10&category=${category}&difficulty=${level}&type=multiple`;
